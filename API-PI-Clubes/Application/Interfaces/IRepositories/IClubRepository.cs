using API_PI_Clubes.Application.DTOs;
using API_PI_Clubes.Model;

namespace API_PI_Clubes.Application.Interfaces.IRepositories
{
    public interface IClubRepository
    {
        Task<(IEnumerable<ResponseClubDTO> Items, int TotalCount)> GetAllAsync(ClubQueryDTO query);
        Task<Club> GetByIdAsync(Guid id);
        Task<List<ResponseClubDTO>> GetAllByAdminIdAsync(Guid id);
        Task<Club?> GetByIdWithImagesAsync(Guid id);
        Task<ResponseDashboardDTO?> GetDashboardAsync(Guid clubId);
        Task AddAsync(Club entity);
        Task AddClubAdminAsync(ClubAdmin clubAdmin);
        void Update(Club entity);
        Task DeleteAsync(Guid id);
        Task<bool> ExistsAsync(Guid id);
        Task SaveChangesAsync();
        Task<bool> IsOwnedByUserAsync(Guid clubId, Guid userId);
        Task<int> CountByUserIdAsync(Guid userId);
        Task<List<ClubCourtUsageDTO>> GetClubsWithCourtCountByUserIdAsync(Guid userId);
    }
}
